...
### Fixed
- **OIDC log noise when disabled** — Startup and per-request auth checks no longer emit verbose OIDC config dumps when OIDC is not enabled; warnings now only appear when OIDC is enabled but misconfigured
- **Redirect and sign-in event logging** — Removed per-request redirect logging; sign-in events now route through the logger and respect `LOG_LEVEL`

### Changed
- **CI: `:latest` tag now applied on version releases** — Pushing a version tag (`v*.*.*`) now tags the image as `:latest` in addition to the versioned tags, eliminating the previous two-build race condition
- **CI: removed stale `main` branch trigger** — Workflow now only triggers on `master` (the actual default branch)

## [0.12.5] - 2026-03-19

### Added
- **Docker health check** — Container now reports health status via `/api/health` endpoint, enabling orchestration tools and compose to detect when the app is ready
...
### Fixed
- **PWA install prompt ignoring "Don't ask again"** — The `beforeinstallprompt` event handler now re-checks dismiss state before showing the prompt, preventing it from reappearing after service worker updates
- **"Open All" not tracking link clicks** — Clicking "Open all in new tabs" for bookmark and service categories now records a click for each item in analytics
...
### Fixed
- **Forgot password error** - Added resilient fallback for email template builder so password reset emails still send even if template rendering fails

### Changed
- **Test email behavior when SMTP already verified** - Sends a generic "Your SMTP is working" test email instead of another verification email with a link
- **Toast message after test email** - Now shows the server's response message instead of a hardcoded verification instruction
